...
\section{Motivations}
\label{sec:org85bec41}
\begin{frame}[label={sec:org493b370}]{Illustration}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org8e166e1}]{Application}
\begin{itemize}
...
\label{sec:orgeedf05f}
\subsection{Introduction}
\label{sec:org7df731d}
\begin{frame}[label={sec:org426be57}]{Spectral indices}
\begin{itemize}
...
\begin{frame}[label={sec:org5156a75}]{Conventional Indices}
...
\label{sec:orgebb83ab}
\begin{frame}[label={sec:org25dbeb1}]{Normalized difference vegetation index}
...
\label{sec:org73df3ef}
\begin{frame}[label={sec:org3de5f0c}]{University of Pavia}
\begin{columns}
...
\begin{frame}[fragile,label={sec:org184bd79}]{Orfeo-Toolbox}
 \begin{itemize}
...
\begin{frame}[label={sec:orga48b1ce}]{University of Pavia - Spectral Indices}
\begin{columns}
...
\begin{frame}[fragile,label={sec:org8030999}]{Where is the vegetation 1/2 ?}
   \begin{center}
  \begin{tikzpicture}
...
\begin{frame}[label={sec:org086c61c}]{Where is the vegetation 2/2 ?}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{center}
\includegraphics[width=0.6\linewidth]{./figures/university_color.png}
...
\subsection{Question}
\label{sec:org7904777}
\begin{frame}[label={sec:org9870802}]{Could you find the good one ?}
...
\label{sec:org39af540}
\subsection{Unsupervised}
\label{sec:orgc08675e}
\begin{frame}[label={sec:orge896d75}]{Principal Components Analysis}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org87bba27}]{Maximization of the variance 1/2}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org8c0d7dc}]{Maximization of the variance 2/2}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org2da02b6}]{PCA in practice}
\begin{enumerate}
...
\end{enumerate}
\end{frame}
\begin{frame}[fragile,label={sec:org502bad0}]{PCA case study 1/3}
...
\begin{frame}[label={sec:org4777001}]{PCA case study 2/3}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[fragile,label={sec:org6c42e81}]{PCA case study 3/3}
 \begin{minted}[fontsize=\footnotesize,obeytabs=true,tabsize=4,bgcolor=bg]{python}
...
\begin{frame}[label={sec:orgf05315f}]{Kernel PCA}
\begin{itemize}
...
\begin{frame}[label={sec:org7f87a9b}]{Kernel PCA in practice}
\begin{itemize}
...
\begin{frame}[fragile,label={sec:orgfdd29df}]{KPCA case study 1/3}
...
\end{minted}
\end{frame}
\begin{frame}[label={sec:orgf8cf7bb}]{KPCA case study 2/3}
\begin{itemize}
\item Explained variance
\begin{center}
  \begin{tikzpicture}
...
\end{itemize}
\end{frame}
\begin{frame}[fragile,label={sec:orgbca6ce6}]{KPCA case study 3/3}
 \begin{minted}[fontsize=\footnotesize,obeytabs=true,tabsize=4,bgcolor=bg]{python}
...
\label{sec:org6b84de8}
\begin{frame}[label={sec:org07eaa18}]{Fisher's Discriminant Analysis}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org54b14ba}]{FDA Algorithm}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[fragile,label={sec:orgf398f4f}]{FDA case study 1/3}
 \begin{minted}[fontsize=\footnotesize,obeytabs=true,tabsize=4,bgcolor=bg]{python}
import rasterTools as rt
import scipy as sp
...
\end{minted}
\end{frame}
\begin{frame}[label={sec:orgc312761}]{FDA case study 2/3}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[fragile,label={sec:orgc6e702f}]{FDA case study 3/3}
 \begin{minted}[fontsize=\footnotesize,obeytabs=true,tabsize=4,bgcolor=bg]{python}
im,GeoT,Proj = rt.open_data('../Data/university.tif')
[h,w,b]=im.shape
im.shape=(h*w,b)
...
\begin{frame}[label={sec:orgfc70f75}]{Feature selection}
\begin{itemize}
...
\begin{frame}[label={sec:orgb275333}]{Large scale feature selection with GMM}
\begin{itemize}
...
\begin{frame}[label={sec:orgd3ae3f1}]{Algorithm 1/2}
...
\begin{frame}[label={sec:org9a9a97f}]{Algorithm 2/2}
...
\begin{frame}[fragile,label={sec:org515d82a}]{FFFS case study 1/3}
 \begin{minted}[fontsize=\footnotesize,obeytabs=true,tabsize=4,bgcolor=bg]{python}
import rasterTools as rt
import scipy as sp
import npfs as npfs
...
\begin{frame}[label={sec:org74d2d84}]{FFFS case study 2/3}
\begin{itemize}
\item Criterion
  \begin{center}
      \begin{tikzpicture}
...
\begin{frame}[fragile,label={sec:org658ec69}]{KPCA}
 \begin{itemize}
...
\label{sec:org032f2bd}
\begin{frame}[label={sec:org6d834b0}]{Why spatial feature extraction?}
...
\label{sec:org2257302}
\begin{frame}[label={sec:orga911cdd}]{Spatial neighborhood}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgf880378}]{Template filters}
\uline{Steps}:
\begin{enumerate}
...
\centerline{\resizebox{0.5\textwidth}{!}{\begin{tikzpicture}
      ...
      \end{tikzpicture}}}
\end{frame}
\begin{frame}[label={sec:org5862877}]{Some filters}
\begin{itemize}
...
\begin{frame}[label={sec:orgf3bb8a1}]{Template filters in action 2/3}
\begin{center}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org7b8c8ae}]{Template filters in action 3/3}
\begin{center}
\includegraphics[trim=2.944cm 8.832cm 2.944cm 10.304cm, clip=true,width=0.22\linewidth]{./figures/university_pc1.png}
\end{center}
...
\label{sec:org3294db7}
\begin{frame}[label={sec:org9c18a40}]{Dilation and erosion}
\begin{itemize}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org06b0107}]{Effets of structuring elements}
\begin{columns}
\begin{column}{0.3\columnwidth}
\begin{figure}[htbp]
...
\begin{frame}[label={sec:orgdc644da}]{Opening and closing}
\begin{itemize}
\item \uline{Opening}:
\begin{itemize}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:orgcbd479d}]{Opening and closing profile}
\begin{itemize}
...
\label{sec:org95f9bc4}
\begin{frame}[label={sec:orgebb1e31}]{Ordering relation}
\begin{itemize}
...
\subsection{Questions}
\label{sec:orgd7fa9ce}
\begin{frame}[fragile,label={sec:orgbff5f7e}]{Noise filtering}
 \begin{itemize}
