...
\section{Motivations}
\label{sec:orga3c55e9}
\begin{frame}[label={sec:org6a67844}]{Illustration}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org1bf45c9}]{Application}
\begin{itemize}
...
\label{sec:org88e9655}
\subsection{Introduction}
\label{sec:orge0f60fa}
\begin{frame}[label={sec:org89c16ca}]{Spectral indices}
\begin{itemize}
...
\begin{frame}[label={sec:org471df03}]{Conventional Indices}
...
\label{sec:org5a630b7}
\begin{frame}[label={sec:org1aa99e5}]{Normalized difference vegetation index}
...
\label{sec:orgb60db9b}
\begin{frame}[label={sec:orgb12fdfb}]{University of Pavia}
\begin{columns}
...
\begin{frame}[fragile,label={sec:org736806a}]{Orfeo-Toolbox}
 \begin{itemize}
...
\begin{frame}[label={sec:org83d1f05}]{University of Pavia - Spectral Indices}
\begin{columns}
...
\begin{frame}[fragile,label={sec:org8f73f04}]{Where is the vegetation 1/2 ?}
   \begin{center}
  \begin{tikzpicture}
...
\begin{frame}[label={sec:orgb6dc2bf}]{Where is the vegetation 2/2 ?}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{center}
\includegraphics[width=0.6\linewidth]{./figures/university_color.png}
...
\subsection{Question}
\label{sec:orga0a8dcd}
\begin{frame}[label={sec:org394d748}]{Could you find the good one ?}
...
\label{sec:org3845fec}
\subsection{Unsupervised}
\label{sec:org9c2fec7}
\begin{frame}[label={sec:org3f6b6bb}]{Principal Components Analysis}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org2bd2e21}]{Maximization of the variance 1/2}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgc618f5a}]{Maximization of the variance 2/2}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org2653f86}]{PCA in practice}
\begin{enumerate}
...
\end{enumerate}
\end{frame}
\begin{frame}[fragile,label={sec:org1539441}]{PCA case study 1/3}
...
\begin{frame}[label={sec:orge771d8b}]{PCA case study 2/3}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[fragile,label={sec:org78b1666}]{PCA case study 3/3}
 \begin{minted}[fontsize=\footnotesize,obeytabs=true,tabsize=4,bgcolor=bg]{python}
...
\begin{frame}[label={sec:org5cc62b3}]{Kernel PCA}
\begin{itemize}
...
\begin{frame}[label={sec:org3c7880d}]{Kernel PCA in practice}
\begin{itemize}
...
\begin{frame}[fragile,label={sec:orgab64b5f}]{KPCA case study 1/3}
...
\end{minted}
\end{frame}
\begin{frame}[label={sec:orgd3cf497}]{KPCA case study 2/3}
\begin{itemize}
\item Explained variance
\begin{center}
  \begin{tikzpicture}
...
\end{itemize}
\end{frame}
\begin{frame}[fragile,label={sec:org212f67d}]{KPCA case study 3/3}
 \begin{minted}[fontsize=\footnotesize,obeytabs=true,tabsize=4,bgcolor=bg]{python}
...
\label{sec:org2abd10c}
\begin{frame}[label={sec:org685101c}]{Fisher's Discriminant Analysis}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org0fc28a0}]{FDA Algorithm}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[fragile,label={sec:org44798a9}]{FDA case study 1/3}
 \begin{minted}[fontsize=\footnotesize,obeytabs=true,tabsize=4,bgcolor=bg]{python}
import rasterTools as rt
import scipy as sp
...
\end{minted}
\end{frame}
\begin{frame}[label={sec:org6e6a13a}]{FDA case study 2/3}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[fragile,label={sec:orga37aef3}]{FDA case study 3/3}
 \begin{minted}[fontsize=\footnotesize,obeytabs=true,tabsize=4,bgcolor=bg]{python}
im,GeoT,Proj = rt.open_data('../Data/university.tif')
[h,w,b]=im.shape
im.shape=(h*w,b)
...
\begin{frame}[label={sec:orgb5f85e9}]{Feature selection}
\begin{itemize}
...
\begin{frame}[label={sec:org4a28a7c}]{Large scale feature selection with GMM}
\begin{itemize}
...
\begin{frame}[label={sec:org83d4a9b}]{Algorithm 1/2}
...
\begin{frame}[label={sec:org10feb74}]{Algorithm 2/2}
...
\begin{frame}[fragile,label={sec:org6af3a96}]{FFFS case study 1/3}
 \begin{minted}[fontsize=\footnotesize,obeytabs=true,tabsize=4,bgcolor=bg]{python}
import rasterTools as rt
import scipy as sp
import npfs as npfs
...
\begin{frame}[label={sec:org6c47225}]{FFFS case study 2/3}
\begin{itemize}
\item Criterion
  \begin{center}
      \begin{tikzpicture}
...
\begin{frame}[fragile,label={sec:orgffe7b38}]{KPCA}
 \begin{itemize}
...
\label{sec:org7ac2de4}
\begin{frame}[label={sec:org06c03f8}]{Why spatial feature extraction?}
...
\end{tabular}}
\end{frame}
\begin{frame}[label={sec:org55530a2}]{More on this topics}
\begin{center}
...
\label{sec:org2d8ee91}
\begin{frame}[label={sec:org28a77e5}]{Spatial neighborhood}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org9fe327d}]{Template filters}
\uline{Steps}:
\begin{enumerate}
...
\centerline{\resizebox{0.5\textwidth}{!}{\begin{tikzpicture}
      ...
      \end{tikzpicture}}}
\end{frame}
\begin{frame}[label={sec:orgdd70c20}]{Some filters}
\begin{itemize}
...
\begin{frame}[label={sec:org5691e50}]{Template filters in action 2/3}
\begin{center}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org3712582}]{Template filters in action 3/3}
\begin{center}
\includegraphics[trim=2.944cm 8.832cm 2.944cm 10.304cm, clip=true,width=0.22\linewidth]{./figures/university_pc1.png}
\end{center}
...
\label{sec:org9a46a4b}
\begin{frame}[label={sec:org42c9d18}]{Dilation and erosion}
\begin{itemize}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:orgc125d04}]{Effets of structuring elements}
\begin{columns}
\begin{column}{0.3\columnwidth}
\begin{figure}[htbp]
...
\begin{frame}[label={sec:orgf7bb0e4}]{Opening and closing}
\begin{itemize}
\item \uline{Opening}:
\begin{itemize}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:orgc1830d1}]{Opening and closing profile}
\begin{itemize}
...
\label{sec:orgc94a606}
\begin{frame}[label={sec:org690248d}]{Ordering relation}
\begin{itemize}
...
\subsection{Questions}
\label{sec:org79ee717}
\begin{frame}[fragile,label={sec:org4945299}]{Noise filtering}
 \begin{itemize}
